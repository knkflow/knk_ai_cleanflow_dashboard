...
const ymdFromUTC = (y: number, m1: number, d: number) => `${y}-${pad2(m1)}-${pad2(d)}`;
...
const dayToYMD = (day: MonthDay) => normalizeYMD((day as any).dateStr ?? day.date);
...
export function Calendar() {
  ...
  /* ---- renderDay (responsive Typo/Abstände) ---- */
  const renderDay = useCallback((day: MonthDay) => {
    ...
    return (
      <div className={`h-full ${day.isCurrentMonth ? '' : 'opacity-40'} select-none`}>
        {/* Kopfzeile (kleiner auf XS) */}
        <div className="text-[10px] sm:text-xs mb-1 flex items-center gap-1.5 sm:gap-2">
          <span className={day.isToday ? 'font-bold text-white' : day.isCurrentMonth ? 'text-white/70' : 'text-white/40'}>
            {day.date.getDate()}
          </span>
          {isUnavailable && (
            <span className="inline-flex items-center rounded-sm px-1.5 py-[1px] text-[9px] sm:text-[10px] font-semibold bg-red-600/90 text-white">
              Nicht verfügbar
            </span>
          )}
        </div>
        ...
        {day.isCurrentMonth && (
          <div className={`relative text-[10px] sm:text-xs p-1.5 sm:p-2 rounded border transition-shadow ${boxClass}`}>
            {!!primaryText && <div className="truncate text-center">{primaryText}</div>}

            {showNamesList && (
              <ul className="mt-1 space-y-0.5 pl-3 sm:pl-4 list-disc">
              ...
              assignedDetails.length > 0 ? (
                <div className="mt-1 flex items-center justify-center">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-1.5 rounded-md bg-white text-black border border-white/60 hover:bg-white/90 transition-colors touch-manipulation"
                    title="Geplante Einsätze ansehen"
                  >
                    <Building2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                    <span className="text-[10px] sm:text-[11px] font-semibold">Geplante Einsätze</span>
                  </button>
                </div>
              ) : (
                <div
                  className="mt-1.5 sm:mt-2 flex items-center justify-center text-white text-[10px] sm:text-[11px] font-bold"
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8">
      {errorMsg && (
        <div className="mb-3 rounded border border-yellow-500/40 bg-yellow-500/10 p-3 text-yellow-200 text-xs sm:text-sm">
          {errorMsg}
        </div>
      )}

      {/* Filter + Auswahl (mobile grid dichter, Buttons full width) */}
      {cleaners.length > 0 && (
        <div className="mb-4">
          <div className="text-white font-semibold mb-2 text-sm sm:text-base">Cleaner auswählen</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-2.5 sm:gap-3">
            ...
            <button
              ...
              className={`rounded-xl px-3 py-2.5 sm:py-3 border transition text-left
                ${isAllView
            ...
            >
              <div className="text-[13px] sm:text-sm font-medium text-white">Alle</div>
              <div className="text-[11px] sm:text-xs text-white/60 mt-0.5">Gesamtübersicht</div>
            </button>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className={`h-6 w-6 sm:h-7 sm:w-7 rounded-lg flex items-center justify-center
                      ${active ? 'bg-white/80 text-black' : 'bg-white/10 text-white/80'}`}>
                      <span className="text-[11px] sm:text-xs font-bold">{initials}</span>
                    </div>
                    <div>
                      <div className="text-[13px] sm:text-sm font-medium text-white truncate">{label}</div>
          ...
          <div className="mt-3 text-[11px] sm:text-xs text-white/60">
            {isAllView
              ...
              : `Ansicht gefiltert auf: ${getCleanerLabel(sortedCleaners.find(x => x.id === selectedCleanerId) as Cleaner)}`}
          </div>
        </div>
      )}

      {/* Kalender (Container: mobile-friendly padding) */}
      ...
      {/* Legende */}
      <div className="mt-4 sm:mt-6 bg-white/5 border border-white/10 p-3 sm:p-4 rounded-lg text-[12px] sm:text-sm text-white/80">
        <h3 className="font-semibold mb-2 sm:mb-3 text-white">Legende</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded border bg-red-500/20 border-red-500/40"></div>
            {isAllView ? 'Mindestens eine Reinigungskraft abwesend' : 'Cleaner abwesend (Details über Button)'}
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded border bg-emerald-500/15 border-emerald-500/35"></div>
      ...
      {/* Modal */}
      {modalOpen && (
        <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-2 sm:p-4">
          <div className="relative w-full max-w-full sm:max-w-xl bg-neutral-900 text-white border border-white/15 rounded-t-2xl sm:rounded-2xl shadow-2xl">
            <div className="flex items-center justify-between p-3 sm:p-4 border-b border-white/10">
              <div className="flex items-center gap-2">
                <Building2 className="w-5 h-5 sm:w-6 sm:h-6" />
                <h4 className="text-base sm:text-lg font-semibold">Geplante Einsätze am {modalDate}</h4>
              </div>
              <button
                type="button"
              ...
              {modalItems.length === 0 ? (
                ...
                <ul className="space-y-2.5 sm:space-y-3">
                  {modalItems
                    ...
                    .map((it, i) => (
                      <li key={i} className="rounded-lg border border-white/10 bg-white/5 p-2.5 sm:p-3">
                        <div className="flex items-center gap-2">
                          <Building2 className="w-4 h-4 sm:w-5 sm:h-5" />
                          <div className="font-medium text-white text-sm sm:text-base">{it.name}</div>
                        </div>
                        {it.address && (
            ...
            <div className="p-2.5 sm:p-3 border-t border-white/10 flex justify-end">
              <button
                type="button"
                onClick={() => setModalOpen(false)}
